...
uint32_t* readFileBinary(const char* filename, size_t* file_size) {
    ...
    *file_size = ftell(file_ptr);
    ...
    uint32_t* file_buffer = malloc(*file_size * sizeof(uint32_t));
...
const char** getRequiredExtensions(uint32_t* requiredExtensionCount) {
    ...
    const char** requiredExtensions = malloc((glfwExtensionCount + additionalRequiredExtensionCount) * sizeof(const char*));
    for (int i = 0; i < glfwExtensionCount + 1; i++) {
        // Does this needs to be freed?
        requiredExtensions[i] = malloc(strlen(glfwExtensions[i]+1));
        requiredExtensions[i] = glfwExtensions[i];
    }

    // Does this needs to be freed?
    requiredExtensions[glfwExtensionCount] = malloc(strlen(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME)+1);
    requiredExtensions[glfwExtensionCount] = VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME;

    // Does this needs to be freed?
    requiredExtensions[glfwExtensionCount+1] = malloc(strlen(VK_EXT_DEBUG_UTILS_EXTENSION_NAME)+1);
    requiredExtensions[glfwExtensionCount+1] = VK_EXT_DEBUG_UTILS_EXTENSION_NAME;

    requiredExtensions[glfwExtensionCount+2] = malloc(strlen(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME)+1);
...
bool createLogicalDevice() {
    ...
    uint32_t* uniqueQueueFamilies = malloc(0);
    ...
    for (int i = 0; i < queueFamilyCount; i++) {
        ...
        if (!in) {
            ...
            uniqueQueueFamilies = realloc(uniqueQueueFamilies, uniqueQueueFamilyCount);
